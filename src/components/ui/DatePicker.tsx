...
export function DatePicker({
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-200 z-50 p-4 w-80">
          {/* Month Navigation */}          <div className="flex items-center justify-between mb-4">
            <button
            ...
            {days.map((day, index) => (
              <button
                ...
                className={`
                  ...
                  ${isToday(day) && !isSelected(day) ? 'border-2 border-blue-600 text-blue-600 bg-blue-50' : ''}
                  ${!isSelected(day) && !isToday(day) && !isDateDisabled(day) ? 'text-gray-900 hover:bg-gray-100' : ''}
                `}
              >
                {day}
              </button>
