...
describe('services page cache sync contract', () => {
  ...
  it('renders service and staff dialogs as full-screen flows on mobile while preserving desktop dialogs', () => {
    ...
    expect(source).toContain('sm:max-w-3xl lg:max-w-5xl');
    expect(source).toContain('grid grid-cols-1 gap-4 lg:grid-cols-2 lg:items-start');
    ...
    expect(source).toContain('space-y-3 xl:grid xl:grid-cols-2 xl:gap-3 xl:space-y-0');
  });
});
