...
export async function POST(
  ...
  try {
    ...
    const checkoutFingerprint = [
      ...
      customerName,
      formattedPhone,
      resolvedServices.map((service) => service.id),
      totals,
    ];
    const idempotencyKey =
      getRequestIdempotencyKey(req) ??
      buildAutomaticIdempotencyKey('deal-checkout', checkoutFingerprint);

    return await runIdempotentJson({
      scope: 'deal-checkout',
      ownerId: deal.business.id,
      key: idempotencyKey,
      ...
      handler: async ({ idempotencyKey: stripeIdempotencyKey }) => {
        ...
        if (!isConnectAccountReady(connectAccount)) {
          return {
            body: { error: 'This business is not ready to accept purchased deals yet' },
            status: 409,
          };
        }
        ...
        return { body: { url: checkoutSession.url, purchaseId: purchase.id } };
      },
    });
  } catch (error: any) {
