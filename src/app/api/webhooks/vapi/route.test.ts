...
describe('POST /api/webhooks/vapi', () => {
  ...
  it('adds a family law intake template without losing the main receptionist workflow', () => {
    const assistant = buildAssistantConfig({
      ...BASE_BUSINESS,
      name: 'Nguyen Family Law',
      businessType: 'Family Law',
      aiReceptionistPhone: '+19087272437',
      services: [
        { id: 'svc-consult', name: 'Initial Consultation', price: 150, duration: 45 },
        { id: 'svc-custody', name: 'Child Custody Consultation', price: 200, duration: 60 },
      ],
      staff: [
        {
          id: 'staff-attorney-1',
    ...
    const toolNames = assistant.model.tools.map((tool: any) => tool?.function?.name ?? tool.type);

    expect(systemPrompt).toContain('Family law intake template');
    expect(systemPrompt).toContain('Keep the same Clientific receptionist workflow');
    expect(systemPrompt).toContain('schedule consultations or appointments');
    expect(systemPrompt).toContain('check availability, reschedule, cancel');
    expect(systemPrompt).toContain('divorce, legal separation, child custody');
    expect(systemPrompt).toContain('child support, spousal support or alimony');
    expect(systemPrompt).toContain('must not give legal advice');
    expect(systemPrompt).toContain('attorney-client relationship');
    expect(systemPrompt).toContain('Social Security numbers');
    expect(systemPrompt).toContain('call emergency services right away');
    expect(systemPrompt).toContain('court hearing, filing deadline, service deadline');
    expect(systemPrompt).toContain('criminal law, immigration, bankruptcy, tax');
    expect(systemPrompt).toContain('Our team focuses on family law');
    expect(systemPrompt).toContain('Do not answer the non-family-law question yourself');
    expect(systemPrompt).toContain('Handle appointment booking, availability, rescheduling, cancellations');
    expect(systemPrompt).toContain('serviceIds for every requested service');
    expect(toolNames).toEqual(expect.arrayContaining(['manage_booking', 'transferCall', 'endCall']));
  });

  it('does not add family law legal fallback copy for non-family-law businesses', () => {
    const assistant = buildAssistantConfig(BASE_BUSINESS as any);
    const systemPrompt = assistant.model.messages[0].content as string;

    expect(systemPrompt).not.toContain('Family law intake template');
    expect(systemPrompt).not.toContain('Our team focuses on family law');
    expect(systemPrompt).not.toContain('criminal law, immigration, bankruptcy, tax');
  });

  it('keeps the family law guardrails on forced-language call paths', () => {
    const assistant = buildAssistantConfig(
      {
        ...BASE_BUSINESS,
        businessType: 'Divorce Law',
        aiReceptionistSpanishEnabled: true,
      } as any,
      { forcedLanguage: 'es' },
    );
