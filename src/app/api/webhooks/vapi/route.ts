...
- Vapi may surface keypad selections in the transcript as phrases like "Press User's Keypad Entry: 1", "User's Keypad Entry: 2", or "Wrangler Press User's Keypad Entry: 1". Treat 1 as English and 2 as Spanish immediately.
...
function buildSharedReceptionistOperationalRules({
  ...
  const preserveAnsweredBookingFieldsRule = compact
    ? '- If the caller already gave the date, time, service, name, or staff preference, keep that information and do not ask for it again unless it was unclear, contradicted, or they changed it. When only one detail is missing, ask only for that missing detail.'
    : '- If the caller already gave the date, time, service, name, or staff preference, keep that information and do not ask for it again unless it was unclear, contradicted, or they changed it.\n- When only one booking detail is missing or unclear, ask only for that missing detail instead of re-asking for the full booking.\n- If the caller answers a clarification with multiple useful details at once, keep all of them and move forward instead of repeating questions you already have the answer to.\n- If you ask a narrow clarification question, like whether a pedicure is regular or gel, and the caller replies with a different detail such as their name, keep that new detail but continue asking only for the still-missing service clarification.';
  ...
  if (compact) {
    return [
      ...
      preserveAnsweredBookingFieldsRule,
      ...
      '- After they confirm, call createBooking with the chosen serviceIds, exact slotTime, staffId if used, customerName only when needed, and notes for any special requests they already mentioned. Do not call createBooking until they confirm.',
      '- After booking, tell the caller the appointment has been requested, not confirmed, and that they will get a text letting them know whether the business accepts it. Then ask if they need anything else. For existing bookings, use getAppointments before cancelAppointment or updateAppointment when needed. Never cancel just to reschedule.',
      '- If they ask for a human, manager, or real person, say exactly "Let me connect you now." Then call transferCall.',
